...
**Confirmed working on Workers:** Pillow (PIL), LangChain (`langchain_core`, `langchain_openai` — needs `rules` config for JS bundling), Pygments, feedparser, httpx, Jinja2, bleach, beautifulsoup4 (without lxml backend).
...
For more features (connection pooling, timeouts, mocking with `respx`), use `httpx`:
